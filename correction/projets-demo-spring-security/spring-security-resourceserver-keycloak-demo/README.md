...
Le client demande un token jwt à keycloak, exemple (à importer sur postman) : 
	curl -X POST "http://localhost:8080/realms/VOTRE_REALM/protocol/openid-connect/token" -H "Content-Type: application/x-www-form-urlencoded" -d "client_id=VOTRE_CLIENT_ID" -d "grant_type=password" -d "username=VOTRE_USER_" -d "password=LE_PWD_DU_USER"
