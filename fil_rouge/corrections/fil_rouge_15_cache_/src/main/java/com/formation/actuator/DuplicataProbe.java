...
public class DuplicataProbe implements HealthIndicator {
	...
	public Health health() {
		...
		if (totalDuplicatas <= SEUIL) {
			statusCheckDuplicatas.put("CheckTotalDuplicatas",
					"Le nombre de duplicatas est trop bas !  Nombre actuel " + totalDuplicatas);
			return Health.down().withDetails(statusCheckDuplicatas).build();
		} else {
			statusCheckDuplicatas.put("TotalDuplicatasOk",
					"Le nombre de duplicatas est correct  Nombre actuel " + totalDuplicatas);
